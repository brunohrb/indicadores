...
- **jun/2026**: **Reajuste = "Valor Recebido" (não mais "Valor Aplicado")**. O usuário queria que `Reajuste Contratos PF/PJ` refletisse o card **"Valor Recebido"** do relatório "Dashboard de Reajustes", não o reajuste aplicado. Fórmula final em `sync-reajustes.js` `daxDeCard`: `SUM('fReajustes'[Valor_Reajustado])` onde `Status_Reajuste = "Pago"`, mês pela coluna **`Data_Pagamento_MINX`** (data do 1º título pago = "Mês Recebido"), classificado PF/PJ por **`dFilial[Tipo_Pessoa]`** (valores `"PF"/"PJ"/"Outros"` — NÃO confundir com `dCliente[tipo_pessoa]` que é `"F"/"J"` e dá valores diferentes). **SEM offset de mês** (a nota antiga de "-1 mês" e de `filial_id` está obsoleta). Dataset/workspace = `e97a6d33` / `e8de7e89` (Diretoria) — o app só tem acesso a esse; o relatório "oficial" fica no workspace `336675a7` que o service principal NÃO acessa, mas o dataset `e97a6d33` é um espelho com as mesmas tabelas (`fReajustes`, `dFilial`, `dCalendario Pagamento`, medidas `Valor Recebido Total` etc). Confirmado batendo: jun/2026 PJ = 691,75 (igual ao relatório). Descoberta feita com script temporário `descobrir-reajustes.js` (removido depois).
