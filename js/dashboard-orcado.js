...
function renderDashboardOrcado() {
  try {
    ...
    if (!el) {
      console.log('⚠️ orcadoView não existe');
      return;
    }
    if (typeof dadosFinanceiros === 'undefined') {
      console.log('⚠️ dadosFinanceiros não está definido');
      return;
    }
    ...
    if (!real[cat] || real[cat].length === 0) {
      console.log('⚠️ Sem dados para categoria:', cat);
      return;
    }

    console.log('✅ Renderizando Dashboard Orçado para:', cat);
...
async function carregarOrcadoDoXLSXBytes(arrayBuffer) {
  try {
    ...
    console.log('✅ Orçamento carregado do XLSX');
...
console.log('🎯 dashboard-orcado.js carregado');
