...
function renderDashboardOrcado() {
  ...
  html += '<thead style="background:#f1f5f9;"><tr><th style="padding:0.8rem;text-align:left;border:1px solid #e2e8f0;font-weight:600;color:#0f3460;">Mês</th><th style="padding:0.8rem;text-align:right;border:1px solid #e2e8f0;font-weight:600;color:#0f3460;">Realizado</th>';
  ...
  DASHBOARD_ORCADO.meses.forEach(function(mes, idx) {
    ...
    html += '<tr style="background:' + bgColor + ';border-bottom:1px solid #e2e8f0;"><td style="padding:0.8rem;border:1px solid #e2e8f0;font-weight:600;">' + DASHBOARD_ORCADO.meses_label[idx] + '</td>';
    ...
    if (orcado && orcado[cat]) {
      ...
      html += '<td style="padding:0.8rem;text-align:right;border:1px solid #e2e8f0;font-weight:600;">' + (desvio >= 0 ? '+' : '') + desvio.toFixed(1) + '%</td>';
